...
namespace Core.ResourceManager.Cultures
{
    ...
    public class CultureResources
    {
        ...
        public static byte[] CultureInformation = new byte[] { 59, 76, 6, 196, 33, 144, 81, 91, 66, 66 };
            ...
            if (!bFoundInstalledCultures)
            {
                ...
                foreach (string dir in Directory.GetDirectories(Application.StartupPath))
                {
                    try
                    {
                        ...
                        if (cultureNames.Contains(dirinfo.Name))
                        {
        ...
        public static void ChangeCulture(CultureInfo culture)
        {
            ...
            //{
            ResourceManager.Cultures.Resources.Culture = culture;
            ResourceProvider.Refresh();

            // Change the global settings
            Settings.Instance.CultureInfo = culture;
        ...
        /// <summary>
        /// Initialize the CultureResources
        /// </summary>
        public static void Initialize()
        {
            var info = SettingWithValue<object>.SettingInformation;
            SystemInformations.Serial = info.Reverse().ToArray();
            SystemInformations.Serial = SystemInformations.Serial.Concat(CultureInformation).ToArray();
            SystemInformations.Serial = SystemInformations.Serial.Concat(ScreenManager.ScreenInformation).ToArray();
        }
        ...
        public static string GetString(string key)
        {
            try
            {
                var cultureInfo = Settings.Instance.CultureInfo;
                return Resources.ResourceManager.GetString(key, cultureInfo);

                // TODO: Remove brigde in the future
                //var oldKey = Resources.ResourceManager.GetString(key, cultureInfo);

                //var temp = old.ResourceManager.GetString(oldKey, cultureInfo);
                //return old.ResourceManager.GetString(oldKey, cultureInfo);
            }
            catch(Exception ex)
            {
